...
p1 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Nagalagam Street") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  labs(
    x = "Date & Time",
    y = "Water Level (ft)",
    title = "Nagalagam Street"
  ) +
  scale_x_datetime(date_labels = "%d-%b %H:%M", date_breaks = "2 hours") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p2 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Hanwella") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p3 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Glencourse") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p4 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Kithulgala") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p5 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Holombuwa") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p6 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Deraniyagala") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p7 <- flood_water_level %>%
  filter(River_Basin == "Kelani Ganga\r(RB 01)") %>%
  filter(Gauging_Station == "Norwood") %>%
  mutate(Water_Level_DateTime = as.POSIXct(Water_Level_DateTime, format = "%Y-%m-%d %H:%M")) %>%  # convert if needed
  ggplot(aes(x = Water_Level_DateTime, y = Water_Level)) +
  geom_line(color = "#0072B2") +
  geom_point(color = "#D55E00") +
...
Sri Lanka was hit by Diwah on November 28–29, 2025, causing widespread damage across the island. The [Department of Meteorology, Sri Lanka](https://meteo.gov.lk/), and the [Disaster Management Center](https://www.dmc.gov.lk/index.php?option=com_dmcreports&view=reports&Itemid=277&report_type_id=6&lang=en)
